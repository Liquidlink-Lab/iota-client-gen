...
export function isClaimPass(type: string): boolean { type = compressIotaType(type); return type === `${PKG_V1}::vault::ClaimPass`; }
...
export class ClaimPass implements StructClass { __StructClass = true as const;
 ...
 private constructor(typeArgs: [], fields: ClaimPassFields, ) { this.$fullTypeName = composeIotaType( ClaimPass.$typeName, ...typeArgs ) as `${typeof PKG_V1}::vault::ClaimPass`; this.$typeArgs = typeArgs;

 this.vaultId = fields.vaultId;; this.index = fields.index;; this.receiver = fields.receiver; }

 static reified( ): ClaimPassReified { return { typeName: ClaimPass.$typeName, fullTypeName: composeIotaType( ClaimPass.$typeName, ...[] ) as `${typeof PKG_V1}::vault::ClaimPass`, typeArgs: [ ] as [], isPhantom: ClaimPass.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => ClaimPass.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => ClaimPass.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => ClaimPass.fromBcs( data, ), bcs: ClaimPass.bcs, fromJSONField: (field: any) => ClaimPass.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => ClaimPass.fromJSON( json, ), fromIotaParsedData: (content: IotaParsedData) => ClaimPass.fromIotaParsedData( content, ), fromIotaObjectData: (content: IotaObjectData) => ClaimPass.fromIotaObjectData( content, ), fetch: async (client: IotaClient, id: string) => ClaimPass.fetch( client, id, ), new: ( fields: ClaimPassFields, ) => { return new ClaimPass( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromIotaParsedData( content: IotaParsedData ): ClaimPass { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isClaimPass(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a ClaimPass object`); } return ClaimPass.fromFieldsWithTypes( content ); }

 static fromIotaObjectData( data: IotaObjectData ): ClaimPass { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isClaimPass(data.bcs.type)) { throw new Error(`object at is not a ClaimPass object`); }

 return ClaimPass.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return ClaimPass.fromIotaParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }

 static async fetch( client: IotaClient, id: string ): Promise<ClaimPass> { const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching ClaimPass object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isClaimPass(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a ClaimPass object`); }

 return ClaimPass.fromIotaObjectData( res.data ); }

 }

/* ============================== ClaimPassVerifyMessage =============================== */

export function isClaimPassVerifyMessage(type: string): boolean { type = compressIotaType(type); return type === `${PKG_V1}::vault::ClaimPassVerifyMessage`; }
...
export class ClaimPassVerifyMessage implements StructClass { __StructClass = true as const;
 ...
 private constructor(typeArgs: [], fields: ClaimPassVerifyMessageFields, ) { this.$fullTypeName = composeIotaType( ClaimPassVerifyMessage.$typeName, ...typeArgs ) as `${typeof PKG_V1}::vault::ClaimPassVerifyMessage`; this.$typeArgs = typeArgs;

 this.vaultId = fields.vaultId;; this.receiver = fields.receiver; }

 static reified( ): ClaimPassVerifyMessageReified { return { typeName: ClaimPassVerifyMessage.$typeName, fullTypeName: composeIotaType( ClaimPassVerifyMessage.$typeName, ...[] ) as `${typeof PKG_V1}::vault::ClaimPassVerifyMessage`, typeArgs: [ ] as [], isPhantom: ClaimPassVerifyMessage.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => ClaimPassVerifyMessage.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => ClaimPassVerifyMessage.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => ClaimPassVerifyMessage.fromBcs( data, ), bcs: ClaimPassVerifyMessage.bcs, fromJSONField: (field: any) => ClaimPassVerifyMessage.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => ClaimPassVerifyMessage.fromJSON( json, ), fromIotaParsedData: (content: IotaParsedData) => ClaimPassVerifyMessage.fromIotaParsedData( content, ), fromIotaObjectData: (content: IotaObjectData) => ClaimPassVerifyMessage.fromIotaObjectData( content, ), fetch: async (client: IotaClient, id: string) => ClaimPassVerifyMessage.fetch( client, id, ), new: ( fields: ClaimPassVerifyMessageFields, ) => { return new ClaimPassVerifyMessage( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromIotaParsedData( content: IotaParsedData ): ClaimPassVerifyMessage { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isClaimPassVerifyMessage(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a ClaimPassVerifyMessage object`); } return ClaimPassVerifyMessage.fromFieldsWithTypes( content ); }

 static fromIotaObjectData( data: IotaObjectData ): ClaimPassVerifyMessage { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isClaimPassVerifyMessage(data.bcs.type)) { throw new Error(`object at is not a ClaimPassVerifyMessage object`); }

 return ClaimPassVerifyMessage.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return ClaimPassVerifyMessage.fromIotaParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }

 static async fetch( client: IotaClient, id: string ): Promise<ClaimPassVerifyMessage> { const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching ClaimPassVerifyMessage object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isClaimPassVerifyMessage(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a ClaimPassVerifyMessage object`); }

 return ClaimPassVerifyMessage.fromIotaObjectData( res.data ); }

 }

/* ============================== CoinAddedEvent =============================== */

export function isCoinAddedEvent(type: string): boolean { type = compressIotaType(type); return type.startsWith(`${PKG_V1}::vault::CoinAddedEvent` + '<'); }
...
export class CoinAddedEvent<T0 extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 private constructor(typeArgs: [PhantomToTypeStr<T0>], fields: CoinAddedEventFields<T0>, ) { this.$fullTypeName = composeIotaType( CoinAddedEvent.$typeName, ...typeArgs ) as `${typeof PKG_V1}::vault::CoinAddedEvent<${PhantomToTypeStr<T0>}>`; this.$typeArgs = typeArgs;

 this.vaultId = fields.vaultId;; this.index = fields.index;; this.coinValue = fields.coinValue; }

 static reified<T0 extends PhantomReified<PhantomTypeArgument>>( T0: T0 ): CoinAddedEventReified<ToPhantomTypeArgument<T0>> { return { typeName: CoinAddedEvent.$typeName, fullTypeName: composeIotaType( CoinAddedEvent.$typeName, ...[extractType(T0)] ) as `${typeof PKG_V1}::vault::CoinAddedEvent<${PhantomToTypeStr<ToPhantomTypeArgument<T0>>}>`, typeArgs: [ extractType(T0) ] as [PhantomToTypeStr<ToPhantomTypeArgument<T0>>], isPhantom: CoinAddedEvent.$isPhantom, reifiedTypeArgs: [T0], fromFields: (fields: Record<string, any>) => CoinAddedEvent.fromFields( T0, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => CoinAddedEvent.fromFieldsWithTypes( T0, item, ), fromBcs: (data: Uint8Array) => CoinAddedEvent.fromBcs( T0, data, ), bcs: CoinAddedEvent.bcs, fromJSONField: (field: any) => CoinAddedEvent.fromJSONField( T0, field, ), fromJSON: (json: Record<string, any>) => CoinAddedEvent.fromJSON( T0, json, ), fromIotaParsedData: (content: IotaParsedData) => CoinAddedEvent.fromIotaParsedData( T0, content, ), fromIotaObjectData: (content: IotaObjectData) => CoinAddedEvent.fromIotaObjectData( T0, content, ), fetch: async (client: IotaClient, id: string) => CoinAddedEvent.fetch( client, T0, id, ), new: ( fields: CoinAddedEventFields<ToPhantomTypeArgument<T0>>, ) => { return new CoinAddedEvent( [extractType(T0)], fields ) }, kind: "StructClassReified", } }
 ...
 static fromJSON<T0 extends PhantomReified<PhantomTypeArgument>>( typeArg: T0, json: Record<string, any> ): CoinAddedEvent<ToPhantomTypeArgument<T0>> { if (json.$typeName !== CoinAddedEvent.$typeName) { throw new Error("not a WithTwoGenerics json object") }; assertReifiedTypeArgsMatch( composeIotaType(CoinAddedEvent.$typeName, extractType(typeArg)), json.$typeArgs, [typeArg], )

 return CoinAddedEvent.fromJSONField( typeArg, json, ) }

 static fromIotaParsedData<T0 extends PhantomReified<PhantomTypeArgument>>( typeArg: T0, content: IotaParsedData ): CoinAddedEvent<ToPhantomTypeArgument<T0>> { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isCoinAddedEvent(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a CoinAddedEvent object`); } return CoinAddedEvent.fromFieldsWithTypes( typeArg, content ); }

 static fromIotaObjectData<T0 extends PhantomReified<PhantomTypeArgument>>( typeArg: T0, data: IotaObjectData ): CoinAddedEvent<ToPhantomTypeArgument<T0>> { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isCoinAddedEvent(data.bcs.type)) { throw new Error(`object at is not a CoinAddedEvent object`); }
 ...
 return CoinAddedEvent.fromBcs( typeArg, fromB64(data.bcs.bcsBytes) ); } if (data.content) { return CoinAddedEvent.fromIotaParsedData( typeArg, data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }

 static async fetch<T0 extends PhantomReified<PhantomTypeArgument>>( client: IotaClient, typeArg: T0, id: string ): Promise<CoinAddedEvent<ToPhantomTypeArgument<T0>>> { const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching CoinAddedEvent object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isCoinAddedEvent(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a CoinAddedEvent object`); }

 return CoinAddedEvent.fromIotaObjectData( typeArg, res.data ); }

 }

/* ============================== CoinGivenEvent =============================== */

export function isCoinGivenEvent(type: string): boolean { type = compressIotaType(type); return type.startsWith(`${PKG_V1}::vault::CoinGivenEvent` + '<'); }
...
export class CoinGivenEvent<T0 extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 private constructor(typeArgs: [PhantomToTypeStr<T0>], fields: CoinGivenEventFields<T0>, ) { this.$fullTypeName = composeIotaType( CoinGivenEvent.$typeName, ...typeArgs ) as `${typeof PKG_V1}::vault::CoinGivenEvent<${PhantomToTypeStr<T0>}>`; this.$typeArgs = typeArgs;
 ...
 static reified<T0 extends PhantomReified<PhantomTypeArgument>>( T0: T0 ): CoinGivenEventReified<ToPhantomTypeArgument<T0>> { return { typeName: CoinGivenEvent.$typeName, fullTypeName: composeIotaType( CoinGivenEvent.$typeName, ...[extractType(T0)] ) as `${typeof PKG_V1}::vault::CoinGivenEvent<${PhantomToTypeStr<ToPhantomTypeArgument<T0>>}>`, typeArgs: [ extractType(T0) ] as [PhantomToTypeStr<ToPhantomTypeArgument<T0>>], isPhantom: CoinGivenEvent.$isPhantom, reifiedTypeArgs: [T0], fromFields: (fields: Record<string, any>) => CoinGivenEvent.fromFields( T0, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => CoinGivenEvent.fromFieldsWithTypes( T0, item, ), fromBcs: (data: Uint8Array) => CoinGivenEvent.fromBcs( T0, data, ), bcs: CoinGivenEvent.bcs, fromJSONField: (field: any) => CoinGivenEvent.fromJSONField( T0, field, ), fromJSON: (json: Record<string, any>) => CoinGivenEvent.fromJSON( T0, json, ), fromIotaParsedData: (content: IotaParsedData) => CoinGivenEvent.fromIotaParsedData( T0, content, ), fromIotaObjectData: (content: IotaObjectData) => CoinGivenEvent.fromIotaObjectData( T0, content, ), fetch: async (client: IotaClient, id: string) => CoinGivenEvent.fetch( client, T0, id, ), new: ( fields: CoinGivenEventFields<ToPhantomTypeArgument<T0>>, ) => { return new CoinGivenEvent( [extractType(T0)], fields ) }, kind: "StructClassReified", } }
 ...
 static fromJSON<T0 extends PhantomReified<PhantomTypeArgument>>( typeArg: T0, json: Record<string, any> ): CoinGivenEvent<ToPhantomTypeArgument<T0>> { if (json.$typeName !== CoinGivenEvent.$typeName) { throw new Error("not a WithTwoGenerics json object") }; assertReifiedTypeArgsMatch( composeIotaType(CoinGivenEvent.$typeName, extractType(typeArg)), json.$typeArgs, [typeArg], )

 return CoinGivenEvent.fromJSONField( typeArg, json, ) }

 static fromIotaParsedData<T0 extends PhantomReified<PhantomTypeArgument>>( typeArg: T0, content: IotaParsedData ): CoinGivenEvent<ToPhantomTypeArgument<T0>> { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isCoinGivenEvent(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a CoinGivenEvent object`); } return CoinGivenEvent.fromFieldsWithTypes( typeArg, content ); }

 static fromIotaObjectData<T0 extends PhantomReified<PhantomTypeArgument>>( typeArg: T0, data: IotaObjectData ): CoinGivenEvent<ToPhantomTypeArgument<T0>> { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isCoinGivenEvent(data.bcs.type)) { throw new Error(`object at is not a CoinGivenEvent object`); }

 const gotTypeArgs = parseTypeName(data.bcs.type).typeArgs; if (gotTypeArgs.length !== 1) { throw new Error(`type argument mismatch: expected 1 type argument but got '${gotTypeArgs.length}'`); }; const gotTypeArg = compressIotaType(gotTypeArgs[0]); const expectedTypeArg = compressIotaType(extractType(typeArg)); if (gotTypeArg !== compressIotaType(extractType(typeArg))) { throw new Error(`type argument mismatch: expected '${expectedTypeArg}' but got '${gotTypeArg}'`); };

 return CoinGivenEvent.fromBcs( typeArg, fromB64(data.bcs.bcsBytes) ); } if (data.content) { return CoinGivenEvent.fromIotaParsedData( typeArg, data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }

 static async fetch<T0 extends PhantomReified<PhantomTypeArgument>>( client: IotaClient, typeArg: T0, id: string ): Promise<CoinGivenEvent<ToPhantomTypeArgument<T0>>> { const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching CoinGivenEvent object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isCoinGivenEvent(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a CoinGivenEvent object`); }

 return CoinGivenEvent.fromIotaObjectData( typeArg, res.data ); }

 }

/* ============================== CoinWithdrawnEvent =============================== */

export function isCoinWithdrawnEvent(type: string): boolean { type = compressIotaType(type); return type.startsWith(`${PKG_V1}::vault::CoinWithdrawnEvent` + '<'); }
...
export class CoinWithdrawnEvent<T0 extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 private constructor(typeArgs: [PhantomToTypeStr<T0>], fields: CoinWithdrawnEventFields<T0>, ) { this.$fullTypeName = composeIotaType( CoinWithdrawnEvent.$typeName, ...typeArgs ) as `${typeof PKG_V1}::vault::CoinWithdrawnEvent<${PhantomToTypeStr<T0>}>`; this.$typeArgs = typeArgs;

 this.vaultId = fields.vaultId;; this.index = fields.index;; this.coinValue = fields.coinValue; }

 static reified<T0 extends PhantomReified<PhantomTypeArgument>>( T0: T0 ): CoinWithdrawnEventReified<ToPhantomTypeArgument<T0>> { return { typeName: CoinWithdrawnEvent.$typeName, fullTypeName: composeIotaType( CoinWithdrawnEvent.$typeName, ...[extractType(T0)] ) as `${typeof PKG_V1}::vault::CoinWithdrawnEvent<${PhantomToTypeStr<ToPhantomTypeArgument<T0>>}>`, typeArgs: [ extractType(T0) ] as [PhantomToTypeStr<ToPhantomTypeArgument<T0>>], isPhantom: CoinWithdrawnEvent.$isPhantom, reifiedTypeArgs: [T0], fromFields: (fields: Record<string, any>) => CoinWithdrawnEvent.fromFields( T0, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => CoinWithdrawnEvent.fromFieldsWithTypes( T0, item, ), fromBcs: (data: Uint8Array) => CoinWithdrawnEvent.fromBcs( T0, data, ), bcs: CoinWithdrawnEvent.bcs, fromJSONField: (field: any) => CoinWithdrawnEvent.fromJSONField( T0, field, ), fromJSON: (json: Record<string, any>) => CoinWithdrawnEvent.fromJSON( T0, json, ), fromIotaParsedData: (content: IotaParsedData) => CoinWithdrawnEvent.fromIotaParsedData( T0, content, ), fromIotaObjectData: (content: IotaObjectData) => CoinWithdrawnEvent.fromIotaObjectData( T0, content, ), fetch: async (client: IotaClient, id: string) => CoinWithdrawnEvent.fetch( client, T0, id, ), new: ( fields: CoinWithdrawnEventFields<ToPhantomTypeArgument<T0>>, ) => { return new CoinWithdrawnEvent( [extractType(T0)], fields ) }, kind: "StructClassReified", } }
 ...
 static fromJSON<T0 extends PhantomReified<PhantomTypeArgument>>( typeArg: T0, json: Record<string, any> ): CoinWithdrawnEvent<ToPhantomTypeArgument<T0>> { if (json.$typeName !== CoinWithdrawnEvent.$typeName) { throw new Error("not a WithTwoGenerics json object") }; assertReifiedTypeArgsMatch( composeIotaType(CoinWithdrawnEvent.$typeName, extractType(typeArg)), json.$typeArgs, [typeArg], )

 return CoinWithdrawnEvent.fromJSONField( typeArg, json, ) }

 static fromIotaParsedData<T0 extends PhantomReified<PhantomTypeArgument>>( typeArg: T0, content: IotaParsedData ): CoinWithdrawnEvent<ToPhantomTypeArgument<T0>> { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isCoinWithdrawnEvent(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a CoinWithdrawnEvent object`); } return CoinWithdrawnEvent.fromFieldsWithTypes( typeArg, content ); }

 static fromIotaObjectData<T0 extends PhantomReified<PhantomTypeArgument>>( typeArg: T0, data: IotaObjectData ): CoinWithdrawnEvent<ToPhantomTypeArgument<T0>> { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isCoinWithdrawnEvent(data.bcs.type)) { throw new Error(`object at is not a CoinWithdrawnEvent object`); }

 const gotTypeArgs = parseTypeName(data.bcs.type).typeArgs; if (gotTypeArgs.length !== 1) { throw new Error(`type argument mismatch: expected 1 type argument but got '${gotTypeArgs.length}'`); }; const gotTypeArg = compressIotaType(gotTypeArgs[0]); const expectedTypeArg = compressIotaType(extractType(typeArg)); if (gotTypeArg !== compressIotaType(extractType(typeArg))) { throw new Error(`type argument mismatch: expected '${expectedTypeArg}' but got '${gotTypeArg}'`); };

 return CoinWithdrawnEvent.fromBcs( typeArg, fromB64(data.bcs.bcsBytes) ); } if (data.content) { return CoinWithdrawnEvent.fromIotaParsedData( typeArg, data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }

 static async fetch<T0 extends PhantomReified<PhantomTypeArgument>>( client: IotaClient, typeArg: T0, id: string ): Promise<CoinWithdrawnEvent<ToPhantomTypeArgument<T0>>> { const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching CoinWithdrawnEvent object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isCoinWithdrawnEvent(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a CoinWithdrawnEvent object`); }

 return CoinWithdrawnEvent.fromIotaObjectData( typeArg, res.data ); }

 }

/* ============================== NumberOfReceiversUpdatedEvent =============================== */

export function isNumberOfReceiversUpdatedEvent(type: string): boolean { type = compressIotaType(type); return type === `${PKG_V1}::vault::NumberOfReceiversUpdatedEvent`; }
...
export class NumberOfReceiversUpdatedEvent implements StructClass { __StructClass = true as const;
 ...
 private constructor(typeArgs: [], fields: NumberOfReceiversUpdatedEventFields, ) { this.$fullTypeName = composeIotaType( NumberOfReceiversUpdatedEvent.$typeName, ...typeArgs ) as `${typeof PKG_V1}::vault::NumberOfReceiversUpdatedEvent`; this.$typeArgs = typeArgs;
 ...
 static reified( ): NumberOfReceiversUpdatedEventReified { return { typeName: NumberOfReceiversUpdatedEvent.$typeName, fullTypeName: composeIotaType( NumberOfReceiversUpdatedEvent.$typeName, ...[] ) as `${typeof PKG_V1}::vault::NumberOfReceiversUpdatedEvent`, typeArgs: [ ] as [], isPhantom: NumberOfReceiversUpdatedEvent.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => NumberOfReceiversUpdatedEvent.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => NumberOfReceiversUpdatedEvent.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => NumberOfReceiversUpdatedEvent.fromBcs( data, ), bcs: NumberOfReceiversUpdatedEvent.bcs, fromJSONField: (field: any) => NumberOfReceiversUpdatedEvent.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => NumberOfReceiversUpdatedEvent.fromJSON( json, ), fromIotaParsedData: (content: IotaParsedData) => NumberOfReceiversUpdatedEvent.fromIotaParsedData( content, ), fromIotaObjectData: (content: IotaObjectData) => NumberOfReceiversUpdatedEvent.fromIotaObjectData( content, ), fetch: async (client: IotaClient, id: string) => NumberOfReceiversUpdatedEvent.fetch( client, id, ), new: ( fields: NumberOfReceiversUpdatedEventFields, ) => { return new NumberOfReceiversUpdatedEvent( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromIotaParsedData( content: IotaParsedData ): NumberOfReceiversUpdatedEvent { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isNumberOfReceiversUpdatedEvent(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a NumberOfReceiversUpdatedEvent object`); } return NumberOfReceiversUpdatedEvent.fromFieldsWithTypes( content ); }

 static fromIotaObjectData( data: IotaObjectData ): NumberOfReceiversUpdatedEvent { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isNumberOfReceiversUpdatedEvent(data.bcs.type)) { throw new Error(`object at is not a NumberOfReceiversUpdatedEvent object`); }

 return NumberOfReceiversUpdatedEvent.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return NumberOfReceiversUpdatedEvent.fromIotaParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }

 static async fetch( client: IotaClient, id: string ): Promise<NumberOfReceiversUpdatedEvent> { const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching NumberOfReceiversUpdatedEvent object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isNumberOfReceiversUpdatedEvent(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a NumberOfReceiversUpdatedEvent object`); }

 return NumberOfReceiversUpdatedEvent.fromIotaObjectData( res.data ); }

 }

/* ============================== ObjectsAddedEvent =============================== */

export function isObjectsAddedEvent(type: string): boolean { type = compressIotaType(type); return type.startsWith(`${PKG_V1}::vault::ObjectsAddedEvent` + '<'); }
...
export class ObjectsAddedEvent<T0 extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 private constructor(typeArgs: [PhantomToTypeStr<T0>], fields: ObjectsAddedEventFields<T0>, ) { this.$fullTypeName = composeIotaType( ObjectsAddedEvent.$typeName, ...typeArgs ) as `${typeof PKG_V1}::vault::ObjectsAddedEvent<${PhantomToTypeStr<T0>}>`; this.$typeArgs = typeArgs;
 ...
 static fromJSON<T0 extends PhantomReified<PhantomTypeArgument>>( typeArg: T0, json: Record<string, any> ): ObjectsAddedEvent<ToPhantomTypeArgument<T0>> { if (json.$typeName !== ObjectsAddedEvent.$typeName) { throw new Error("not a WithTwoGenerics json object") }; assertReifiedTypeArgsMatch( composeIotaType(ObjectsAddedEvent.$typeName, extractType(typeArg)), json.$typeArgs, [typeArg], )

 return ObjectsAddedEvent.fromJSONField( typeArg, json, ) }

 static fromIotaParsedData<T0 extends PhantomReified<PhantomTypeArgument>>( typeArg: T0, content: IotaParsedData ): ObjectsAddedEvent<ToPhantomTypeArgument<T0>> { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isObjectsAddedEvent(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a ObjectsAddedEvent object`); } return ObjectsAddedEvent.fromFieldsWithTypes( typeArg, content ); }

 static fromIotaObjectData<T0 extends PhantomReified<PhantomTypeArgument>>( typeArg: T0, data: IotaObjectData ): ObjectsAddedEvent<ToPhantomTypeArgument<T0>> { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isObjectsAddedEvent(data.bcs.type)) { throw new Error(`object at is not a ObjectsAddedEvent object`); }

 const gotTypeArgs = parseTypeName(data.bcs.type).typeArgs; if (gotTypeArgs.length !== 1) { throw new Error(`type argument mismatch: expected 1 type argument but got '${gotTypeArgs.length}'`); }; const gotTypeArg = compressIotaType(gotTypeArgs[0]); const expectedTypeArg = compressIotaType(extractType(typeArg)); if (gotTypeArg !== compressIotaType(extractType(typeArg))) { throw new Error(`type argument mismatch: expected '${expectedTypeArg}' but got '${gotTypeArg}'`); };

 return ObjectsAddedEvent.fromBcs( typeArg, fromB64(data.bcs.bcsBytes) ); } if (data.content) { return ObjectsAddedEvent.fromIotaParsedData( typeArg, data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }

 static async fetch<T0 extends PhantomReified<PhantomTypeArgument>>( client: IotaClient, typeArg: T0, id: string ): Promise<ObjectsAddedEvent<ToPhantomTypeArgument<T0>>> { const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching ObjectsAddedEvent object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isObjectsAddedEvent(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a ObjectsAddedEvent object`); }

 return ObjectsAddedEvent.fromIotaObjectData( typeArg, res.data ); }

 }

/* ============================== ObjectsGivenEvent =============================== */

export function isObjectsGivenEvent(type: string): boolean { type = compressIotaType(type); return type.startsWith(`${PKG_V1}::vault::ObjectsGivenEvent` + '<'); }
...
export class ObjectsGivenEvent<T0 extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 private constructor(typeArgs: [PhantomToTypeStr<T0>], fields: ObjectsGivenEventFields<T0>, ) { this.$fullTypeName = composeIotaType( ObjectsGivenEvent.$typeName, ...typeArgs ) as `${typeof PKG_V1}::vault::ObjectsGivenEvent<${PhantomToTypeStr<T0>}>`; this.$typeArgs = typeArgs;
 ...
 static fromJSON<T0 extends PhantomReified<PhantomTypeArgument>>( typeArg: T0, json: Record<string, any> ): ObjectsGivenEvent<ToPhantomTypeArgument<T0>> { if (json.$typeName !== ObjectsGivenEvent.$typeName) { throw new Error("not a WithTwoGenerics json object") }; assertReifiedTypeArgsMatch( composeIotaType(ObjectsGivenEvent.$typeName, extractType(typeArg)), json.$typeArgs, [typeArg], )

 return ObjectsGivenEvent.fromJSONField( typeArg, json, ) }

 static fromIotaParsedData<T0 extends PhantomReified<PhantomTypeArgument>>( typeArg: T0, content: IotaParsedData ): ObjectsGivenEvent<ToPhantomTypeArgument<T0>> { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isObjectsGivenEvent(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a ObjectsGivenEvent object`); } return ObjectsGivenEvent.fromFieldsWithTypes( typeArg, content ); }

 static fromIotaObjectData<T0 extends PhantomReified<PhantomTypeArgument>>( typeArg: T0, data: IotaObjectData ): ObjectsGivenEvent<ToPhantomTypeArgument<T0>> { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isObjectsGivenEvent(data.bcs.type)) { throw new Error(`object at is not a ObjectsGivenEvent object`); }

 const gotTypeArgs = parseTypeName(data.bcs.type).typeArgs; if (gotTypeArgs.length !== 1) { throw new Error(`type argument mismatch: expected 1 type argument but got '${gotTypeArgs.length}'`); }; const gotTypeArg = compressIotaType(gotTypeArgs[0]); const expectedTypeArg = compressIotaType(extractType(typeArg)); if (gotTypeArg !== compressIotaType(extractType(typeArg))) { throw new Error(`type argument mismatch: expected '${expectedTypeArg}' but got '${gotTypeArg}'`); };

 return ObjectsGivenEvent.fromBcs( typeArg, fromB64(data.bcs.bcsBytes) ); } if (data.content) { return ObjectsGivenEvent.fromIotaParsedData( typeArg, data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }

 static async fetch<T0 extends PhantomReified<PhantomTypeArgument>>( client: IotaClient, typeArg: T0, id: string ): Promise<ObjectsGivenEvent<ToPhantomTypeArgument<T0>>> { const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching ObjectsGivenEvent object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isObjectsGivenEvent(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a ObjectsGivenEvent object`); }

 return ObjectsGivenEvent.fromIotaObjectData( typeArg, res.data ); }

 }

/* ============================== ObjectsWithdrawnEvent =============================== */

export function isObjectsWithdrawnEvent(type: string): boolean { type = compressIotaType(type); return type.startsWith(`${PKG_V1}::vault::ObjectsWithdrawnEvent` + '<'); }
...
export class ObjectsWithdrawnEvent<T0 extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 private constructor(typeArgs: [PhantomToTypeStr<T0>], fields: ObjectsWithdrawnEventFields<T0>, ) { this.$fullTypeName = composeIotaType( ObjectsWithdrawnEvent.$typeName, ...typeArgs ) as `${typeof PKG_V1}::vault::ObjectsWithdrawnEvent<${PhantomToTypeStr<T0>}>`; this.$typeArgs = typeArgs;

 this.vaultId = fields.vaultId;; this.index = fields.index;; this.totalItems = fields.totalItems; }

 static reified<T0 extends PhantomReified<PhantomTypeArgument>>( T0: T0 ): ObjectsWithdrawnEventReified<ToPhantomTypeArgument<T0>> { return { typeName: ObjectsWithdrawnEvent.$typeName, fullTypeName: composeIotaType( ObjectsWithdrawnEvent.$typeName, ...[extractType(T0)] ) as `${typeof PKG_V1}::vault::ObjectsWithdrawnEvent<${PhantomToTypeStr<ToPhantomTypeArgument<T0>>}>`, typeArgs: [ extractType(T0) ] as [PhantomToTypeStr<ToPhantomTypeArgument<T0>>], isPhantom: ObjectsWithdrawnEvent.$isPhantom, reifiedTypeArgs: [T0], fromFields: (fields: Record<string, any>) => ObjectsWithdrawnEvent.fromFields( T0, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => ObjectsWithdrawnEvent.fromFieldsWithTypes( T0, item, ), fromBcs: (data: Uint8Array) => ObjectsWithdrawnEvent.fromBcs( T0, data, ), bcs: ObjectsWithdrawnEvent.bcs, fromJSONField: (field: any) => ObjectsWithdrawnEvent.fromJSONField( T0, field, ), fromJSON: (json: Record<string, any>) => ObjectsWithdrawnEvent.fromJSON( T0, json, ), fromIotaParsedData: (content: IotaParsedData) => ObjectsWithdrawnEvent.fromIotaParsedData( T0, content, ), fromIotaObjectData: (content: IotaObjectData) => ObjectsWithdrawnEvent.fromIotaObjectData( T0, content, ), fetch: async (client: IotaClient, id: string) => ObjectsWithdrawnEvent.fetch( client, T0, id, ), new: ( fields: ObjectsWithdrawnEventFields<ToPhantomTypeArgument<T0>>, ) => { return new ObjectsWithdrawnEvent( [extractType(T0)], fields ) }, kind: "StructClassReified", } }
 ...
 static fromJSON<T0 extends PhantomReified<PhantomTypeArgument>>( typeArg: T0, json: Record<string, any> ): ObjectsWithdrawnEvent<ToPhantomTypeArgument<T0>> { if (json.$typeName !== ObjectsWithdrawnEvent.$typeName) { throw new Error("not a WithTwoGenerics json object") }; assertReifiedTypeArgsMatch( composeIotaType(ObjectsWithdrawnEvent.$typeName, extractType(typeArg)), json.$typeArgs, [typeArg], )

 return ObjectsWithdrawnEvent.fromJSONField( typeArg, json, ) }

 static fromIotaParsedData<T0 extends PhantomReified<PhantomTypeArgument>>( typeArg: T0, content: IotaParsedData ): ObjectsWithdrawnEvent<ToPhantomTypeArgument<T0>> { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isObjectsWithdrawnEvent(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a ObjectsWithdrawnEvent object`); } return ObjectsWithdrawnEvent.fromFieldsWithTypes( typeArg, content ); }

 static fromIotaObjectData<T0 extends PhantomReified<PhantomTypeArgument>>( typeArg: T0, data: IotaObjectData ): ObjectsWithdrawnEvent<ToPhantomTypeArgument<T0>> { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isObjectsWithdrawnEvent(data.bcs.type)) { throw new Error(`object at is not a ObjectsWithdrawnEvent object`); }

 const gotTypeArgs = parseTypeName(data.bcs.type).typeArgs; if (gotTypeArgs.length !== 1) { throw new Error(`type argument mismatch: expected 1 type argument but got '${gotTypeArgs.length}'`); }; const gotTypeArg = compressIotaType(gotTypeArgs[0]); const expectedTypeArg = compressIotaType(extractType(typeArg)); if (gotTypeArg !== compressIotaType(extractType(typeArg))) { throw new Error(`type argument mismatch: expected '${expectedTypeArg}' but got '${gotTypeArg}'`); };

 return ObjectsWithdrawnEvent.fromBcs( typeArg, fromB64(data.bcs.bcsBytes) ); } if (data.content) { return ObjectsWithdrawnEvent.fromIotaParsedData( typeArg, data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }

 static async fetch<T0 extends PhantomReified<PhantomTypeArgument>>( client: IotaClient, typeArg: T0, id: string ): Promise<ObjectsWithdrawnEvent<ToPhantomTypeArgument<T0>>> { const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching ObjectsWithdrawnEvent object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isObjectsWithdrawnEvent(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a ObjectsWithdrawnEvent object`); }

 return ObjectsWithdrawnEvent.fromIotaObjectData( typeArg, res.data ); }

 }

/* ============================== ObjectsWrapper =============================== */

export function isObjectsWrapper(type: string): boolean { type = compressIotaType(type); return type.startsWith(`${PKG_V1}::vault::ObjectsWrapper` + '<'); }
...
export class ObjectsWrapper<T0 extends TypeArgument> implements StructClass { __StructClass = true as const;
 ...
 private constructor(typeArgs: [ToTypeStr<T0>], fields: ObjectsWrapperFields<T0>, ) { this.$fullTypeName = composeIotaType( ObjectsWrapper.$typeName, ...typeArgs ) as `${typeof PKG_V1}::vault::ObjectsWrapper<${ToTypeStr<T0>}>`; this.$typeArgs = typeArgs;

 this.id = fields.id;; this.itemsPerReceiver = fields.itemsPerReceiver;; this.objects = fields.objects; }

 static reified<T0 extends Reified<TypeArgument, any>>( T0: T0 ): ObjectsWrapperReified<ToTypeArgument<T0>> { return { typeName: ObjectsWrapper.$typeName, fullTypeName: composeIotaType( ObjectsWrapper.$typeName, ...[extractType(T0)] ) as `${typeof PKG_V1}::vault::ObjectsWrapper<${ToTypeStr<ToTypeArgument<T0>>}>`, typeArgs: [ extractType(T0) ] as [ToTypeStr<ToTypeArgument<T0>>], isPhantom: ObjectsWrapper.$isPhantom, reifiedTypeArgs: [T0], fromFields: (fields: Record<string, any>) => ObjectsWrapper.fromFields( T0, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => ObjectsWrapper.fromFieldsWithTypes( T0, item, ), fromBcs: (data: Uint8Array) => ObjectsWrapper.fromBcs( T0, data, ), bcs: ObjectsWrapper.bcs(toBcs(T0)), fromJSONField: (field: any) => ObjectsWrapper.fromJSONField( T0, field, ), fromJSON: (json: Record<string, any>) => ObjectsWrapper.fromJSON( T0, json, ), fromIotaParsedData: (content: IotaParsedData) => ObjectsWrapper.fromIotaParsedData( T0, content, ), fromIotaObjectData: (content: IotaObjectData) => ObjectsWrapper.fromIotaObjectData( T0, content, ), fetch: async (client: IotaClient, id: string) => ObjectsWrapper.fetch( client, T0, id, ), new: ( fields: ObjectsWrapperFields<ToTypeArgument<T0>>, ) => { return new ObjectsWrapper( [extractType(T0)], fields ) }, kind: "StructClassReified", } }
 ...
 static fromJSON<T0 extends Reified<TypeArgument, any>>( typeArg: T0, json: Record<string, any> ): ObjectsWrapper<ToTypeArgument<T0>> { if (json.$typeName !== ObjectsWrapper.$typeName) { throw new Error("not a WithTwoGenerics json object") }; assertReifiedTypeArgsMatch( composeIotaType(ObjectsWrapper.$typeName, extractType(typeArg)), json.$typeArgs, [typeArg], )

 return ObjectsWrapper.fromJSONField( typeArg, json, ) }

 static fromIotaParsedData<T0 extends Reified<TypeArgument, any>>( typeArg: T0, content: IotaParsedData ): ObjectsWrapper<ToTypeArgument<T0>> { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isObjectsWrapper(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a ObjectsWrapper object`); } return ObjectsWrapper.fromFieldsWithTypes( typeArg, content ); }

 static fromIotaObjectData<T0 extends Reified<TypeArgument, any>>( typeArg: T0, data: IotaObjectData ): ObjectsWrapper<ToTypeArgument<T0>> { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isObjectsWrapper(data.bcs.type)) { throw new Error(`object at is not a ObjectsWrapper object`); }

 const gotTypeArgs = parseTypeName(data.bcs.type).typeArgs; if (gotTypeArgs.length !== 1) { throw new Error(`type argument mismatch: expected 1 type argument but got '${gotTypeArgs.length}'`); }; const gotTypeArg = compressIotaType(gotTypeArgs[0]); const expectedTypeArg = compressIotaType(extractType(typeArg)); if (gotTypeArg !== compressIotaType(extractType(typeArg))) { throw new Error(`type argument mismatch: expected '${expectedTypeArg}' but got '${gotTypeArg}'`); };

 return ObjectsWrapper.fromBcs( typeArg, fromB64(data.bcs.bcsBytes) ); } if (data.content) { return ObjectsWrapper.fromIotaParsedData( typeArg, data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }

 static async fetch<T0 extends Reified<TypeArgument, any>>( client: IotaClient, typeArg: T0, id: string ): Promise<ObjectsWrapper<ToTypeArgument<T0>>> { const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching ObjectsWrapper object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isObjectsWrapper(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a ObjectsWrapper object`); }

 return ObjectsWrapper.fromIotaObjectData( typeArg, res.data ); }

 }

/* ============================== Vault =============================== */

export function isVault(type: string): boolean { type = compressIotaType(type); return type === `${PKG_V1}::vault::Vault`; }
...
export class Vault implements StructClass { __StructClass = true as const;
 ...
 private constructor(typeArgs: [], fields: VaultFields, ) { this.$fullTypeName = composeIotaType( Vault.$typeName, ...typeArgs ) as `${typeof PKG_V1}::vault::Vault`; this.$typeArgs = typeArgs;
 ...
 static reified( ): VaultReified { return { typeName: Vault.$typeName, fullTypeName: composeIotaType( Vault.$typeName, ...[] ) as `${typeof PKG_V1}::vault::Vault`, typeArgs: [ ] as [], isPhantom: Vault.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Vault.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Vault.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Vault.fromBcs( data, ), bcs: Vault.bcs, fromJSONField: (field: any) => Vault.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Vault.fromJSON( json, ), fromIotaParsedData: (content: IotaParsedData) => Vault.fromIotaParsedData( content, ), fromIotaObjectData: (content: IotaObjectData) => Vault.fromIotaObjectData( content, ), fetch: async (client: IotaClient, id: string) => Vault.fetch( client, id, ), new: ( fields: VaultFields, ) => { return new Vault( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromIotaParsedData( content: IotaParsedData ): Vault { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isVault(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a Vault object`); } return Vault.fromFieldsWithTypes( content ); }

 static fromIotaObjectData( data: IotaObjectData ): Vault { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isVault(data.bcs.type)) { throw new Error(`object at is not a Vault object`); }

 return Vault.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return Vault.fromIotaParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }

 static async fetch( client: IotaClient, id: string ): Promise<Vault> { const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching Vault object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isVault(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a Vault object`); }

 return Vault.fromIotaObjectData( res.data ); }

 }

/* ============================== VaultCreatedEvent =============================== */

export function isVaultCreatedEvent(type: string): boolean { type = compressIotaType(type); return type === `${PKG_V1}::vault::VaultCreatedEvent`; }
...
export class VaultCreatedEvent implements StructClass { __StructClass = true as const;
 ...
 static fromIotaParsedData( content: IotaParsedData ): VaultCreatedEvent { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isVaultCreatedEvent(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a VaultCreatedEvent object`); } return VaultCreatedEvent.fromFieldsWithTypes( content ); }

 static fromIotaObjectData( data: IotaObjectData ): VaultCreatedEvent { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isVaultCreatedEvent(data.bcs.type)) { throw new Error(`object at is not a VaultCreatedEvent object`); }

 return VaultCreatedEvent.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return VaultCreatedEvent.fromIotaParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }

 static async fetch( client: IotaClient, id: string ): Promise<VaultCreatedEvent> { const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching VaultCreatedEvent object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isVaultCreatedEvent(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a VaultCreatedEvent object`); }

 return VaultCreatedEvent.fromIotaObjectData( res.data ); }

 }

/* ============================== VaultDestroyedEvent =============================== */

export function isVaultDestroyedEvent(type: string): boolean { type = compressIotaType(type); return type === `${PKG_V1}::vault::VaultDestroyedEvent`; }
...
export class VaultDestroyedEvent implements StructClass { __StructClass = true as const;
 ...
 private constructor(typeArgs: [], fields: VaultDestroyedEventFields, ) { this.$fullTypeName = composeIotaType( VaultDestroyedEvent.$typeName, ...typeArgs ) as `${typeof PKG_V1}::vault::VaultDestroyedEvent`; this.$typeArgs = typeArgs;

 this.vaultId = fields.vaultId; }

 static reified( ): VaultDestroyedEventReified { return { typeName: VaultDestroyedEvent.$typeName, fullTypeName: composeIotaType( VaultDestroyedEvent.$typeName, ...[] ) as `${typeof PKG_V1}::vault::VaultDestroyedEvent`, typeArgs: [ ] as [], isPhantom: VaultDestroyedEvent.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => VaultDestroyedEvent.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => VaultDestroyedEvent.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => VaultDestroyedEvent.fromBcs( data, ), bcs: VaultDestroyedEvent.bcs, fromJSONField: (field: any) => VaultDestroyedEvent.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => VaultDestroyedEvent.fromJSON( json, ), fromIotaParsedData: (content: IotaParsedData) => VaultDestroyedEvent.fromIotaParsedData( content, ), fromIotaObjectData: (content: IotaObjectData) => VaultDestroyedEvent.fromIotaObjectData( content, ), fetch: async (client: IotaClient, id: string) => VaultDestroyedEvent.fetch( client, id, ), new: ( fields: VaultDestroyedEventFields, ) => { return new VaultDestroyedEvent( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromIotaParsedData( content: IotaParsedData ): VaultDestroyedEvent { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isVaultDestroyedEvent(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a VaultDestroyedEvent object`); } return VaultDestroyedEvent.fromFieldsWithTypes( content ); }

 static fromIotaObjectData( data: IotaObjectData ): VaultDestroyedEvent { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isVaultDestroyedEvent(data.bcs.type)) { throw new Error(`object at is not a VaultDestroyedEvent object`); }

 return VaultDestroyedEvent.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return VaultDestroyedEvent.fromIotaParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }

 static async fetch( client: IotaClient, id: string ): Promise<VaultDestroyedEvent> { const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching VaultDestroyedEvent object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isVaultDestroyedEvent(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a VaultDestroyedEvent object`); }

 return VaultDestroyedEvent.fromIotaObjectData( res.data ); }
